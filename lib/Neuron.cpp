...
double Neuron::doActivationPrime(double _input){
    ...
    switch(actMet){
        case 0:
            result = (doActivation(_input) + 0.5) * (0.5 - doActivation(_input)); //exp(-_input) / pow((exp(-_input) + 1),2);
            break;
        case 1:
...
void Neuron::initLearningAndMomentum(double _learningRate, double _momentum){
    learningRate=_learningRate;
    momentum = _momentum;
}
...
void Neuron::updateLearningandMomentum(){
    double u = 0.0001;
    double v = 0.001;
    if (error >= 0){
        momentum = momentum*(1 - u*exp(-error));
        learningRate = learningRate*(1 - v*exp(-error));
    } else {
        momentum = momentum*(1 + u*exp(-error));
        learningRate = learningRate*(1 + v*exp(-error));
    }
